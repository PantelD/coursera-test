...
class LineAnalyzer
  ...
  def calculate_word_frequency
    ...
    content.downcase.split.each { |word| frequency[word] += 1 }
    
    @highest_wf_count = frequency.values.max
    @highest_wf_words = frequency.keys.select { |key| frequency[key] == @highest_wf_count}
  end
end
...
class Solution
  ...
  def analyze_file
    my_file = File.new("test.txt")
    array_of_lines = IO.readlines(my_file)
  ...
  def print_highest_word_frequency_across_lines
    @highest_count_words_across_lines.each do |analyzer|
      print analyzer.highest_wf_words, ' ', "(appears in line #{analyzer.line_number})"
      puts
    end
  end
  self
...
my_test = Solution.new
my_test.analyze_file.calculate_line_with_highest_frequency.print_highest_word_frequency_across_lines